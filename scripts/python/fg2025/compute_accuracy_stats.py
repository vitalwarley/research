...
def get_experiment_accuracies(n_experiments: int = 100) -> pd.DataFrame:
    ...
    try:
        ...
        result = subprocess.run(guild_command, check=True, capture_output=True, text=True)
...
def main():
    parser = argparse.ArgumentParser(description='Compute accuracy statistics from Guild experiments')
    parser.add_argument('-n', '--num-experiments', type=int, default=100,
