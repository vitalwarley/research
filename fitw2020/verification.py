...
def prepare_images(root_dir: Path, output_dir: Path) -> None:
    ...
    for family_path in tqdm(root_dir.iterdir()):
        for person_path in family_path.iterdir():
            if not person_path.is_dir() or not person_path.name.startswith(
                whitelist_dir
            ):
                continue
...
class CompareModel(object):
    ...
    def prepare_all_embeddings(self, paths: List[Path], **loader_params) -> None:
        data = mx.gluon.data.DataLoader(
            ImgDataset(paths), shuffle=False, **loader_params
        )
...
if __name__ == "__main__":
    ...
    fpr, tpr, _ = roc_curve(y_true, predictions)
    plt.plot(
        fpr, tpr, color="g", lw=2, label=f" +classification AUC:{auc(fpr, tpr):.4f}"
    )
    ...
    fpr, tpr, _ = roc_curve(y_true, predictions)
    plt.plot(
        fpr, tpr, color="r", lw=2, label=f" +normalization AUC:{auc(fpr, tpr):.4f}"
    )
