...
from dataset import ageKinFaceByFolder
...
def cross_validate(config):
    ...
    for folder in ['father-dau', 'father-son', 'mother-dau', 'mother-son']:
        ...
        for fold in range(1, 6):
            train_dataset = ageKinFaceByFolder(config['data_path'], folder, fold, transform=adaface_transform, train=True, kinface_version='I')
            train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], num_workers=4, pin_memory=True)

            test_dataset = ageKinFaceByFolder(config['data_path'], folder, fold, transform=adaface_transform, kinface_version='I')
